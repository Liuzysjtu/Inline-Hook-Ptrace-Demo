...
int main(int argc, char *argv[]) {
    char InjectModuleName[MAX_PATH] = "/data/libIHook64.so";    // 注入模块全路径
    char RemoteCallFunc[MAX_PATH] = "MainActivity";              // 注入模块后调用模块函数名称
    char InjectProcessName[MAX_PATH] = "com.example.ibored";                      // 注入进程名称
    ...
    LOGD("Current Environment ARM32");
#elif defined(__aarch64__)
    LOGD("Current Environment ARM64");
#else
