...
int fixPCOpcodeArm(void *fixOpcodes , THUMB_INLINE_HOOK_INFO* pstInlineHook)
{
    uint32_t pc;
    uint32_t lr;
    int backUpPos = 0;
    ...
    pc = (uint32_t) (pstInlineHook->pHookAddr + 8); //pc变量用于保存原本指令执行时的pc值
    lr = (uint32_t) (pstInlineHook->pHookAddr + pstInlineHook->backUpLength);
...
int fixPCOpcodeThumb(void *fixOpcodes , THUMB_INLINE_HOOK_INFO* pstInlineHook)
{
    uint32_t pc;
    uint32_t lr;
    int backUpPos = 0;
    ...
    pc = (uint32_t)(pstInlineHook->pHookAddr - 1 + 4); //pc变量用于保存原本指令执行时的pc值
    ...
    lr = (uint32_t)(pstInlineHook->pHookAddr + backUpPos);
...
int fixPCOpcodeThumb16(uint32_t pc, uint16_t instruction, uint16_t *trampoline_instructions, THUMB_INLINE_HOOK_INFO* pstInlineHook)
{
    int type;
    int offset;
...
int fixPCOpcodeArm32(uint32_t pc, uint32_t lr, uint32_t instruction, uint32_t *trampoline_instructions, THUMB_INLINE_HOOK_INFO* pstInlineHook)
{
    int type;
    //int offset;
    int trampoline_pos;
...
int fixPCOpcodeThumb32(uint32_t pc, uint16_t high_instruction, uint16_t low_instruction, uint16_t *trampoline_instructions, THUMB_INLINE_HOOK_INFO* pstInlineHook)
{
